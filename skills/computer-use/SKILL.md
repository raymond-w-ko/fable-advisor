...
- **Foldables carry a posture line.** The iPhone Duo simulator boots folded, and XcodeBuildMCP taps and screenshots target the inner panel, which is off in that posture: taps do nothing and screenshots are black while the accessibility tree looks fine. Unfold it in Device Hub before delegating (no CLI does this) and write `Posture: unfolded` in the brief; the operator refuses a folded device as `contested`. Unfolded, the Duo's inner panel is a landscape split view, so briefs for list-detail apps must accept a pane update rather than a pushed screen. Folded-state checks stay manual for now.
