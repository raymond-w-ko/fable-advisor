...
| Browser / computer use | GPT-6 Astra (effort `medium` by default) | `astra-operator` agent | Anything that needs a real browser or desktop: UI verification, screenshots, login flows, drag-and-drop, browser E2E. Astra is currently the strongest model at driving a browser. Doctrine lives in the `computer-use` skill. |
