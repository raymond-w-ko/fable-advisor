---
name: orchestration
description: Routing doctrine for the architect-as-orchestrator pattern — how a Fable 5.1 session delegates routine implementation to the GPT-5.6 Luna lane, escalates high-complexity one-offs to the GPT-5.6 Sol lane, sends read-only data pulls to the data-investigator lane, runs the Luna lane at `max` always, picks a reasoning effort per task for the Sol lane and the Astra advisor, and gets every deliverable reviewed by the Fable advisor (and, on anything of moderate complexity or above, the Astra advisor) before reporting done. USE WHEN delegating implementation or investigation work, choosing between codex-implementer/sol-implementer/data-investigator lanes, choosing a reasoning effort for a lane, writing a spec for a subagent, deciding whether to consult fable-advisor or astra-advisor, using the Codex plugin's review skills, managing session cost or token spend, or running any multi-task build where the session is the architect.
---

# Orchestration — the architect's routing doctrine

The session is the architect: it owns requirements, architecture, decomposition, specs, routing, verification, and the written findings. It should almost never type implementation code, and it should almost never hold raw data in its context. Every implementation task gets routed to the cheapest lane that is adequate for it; the routine Luna lane always runs at `max`, and escalation to Sol, at an effort chosen per task, is deliberate and never a fixed binding — every data pull goes to the investigation lane, and every finished deliverable gets a Fable review before the architect reports done.
...
|---|---|---|---|
| Routine implementation | GPT-5.6 Luna (always `max`) | `codex-implementer` agent | The spec fully determines the outcome: boilerplate, wiring, CRUD, mechanical edits, straightforward features. **Default implementation lane.** Requires the codex CLI. |
...
The Luna lane is pinned at `max`: `codex-implementer` runs `gpt-5.6-luna` at `max` on every task regardless of the spec's `REASONING` line (rationale below). For Sol and the Astra advisor the architect names one effort per task in the spec, and the lane passes it through unchanged. Pick the lowest rung that is adequate there; on those models effort is real cost and wall-clock, not a quality dial to leave at max.

| Rung | Sol | Astra advisor | Use for |
|---|---|---|---|
...
**Why Luna is pinned at `max` (v5.4).** Effort on Luna is a token-count dial, not a price tier: the model bills the same per-token rate at every effort, and the absolute dollars are small enough that climbing the whole ladder from `low` to `max` adds about three cents per task. Third-party measurements on the Artificial Analysis Intelligence Index, as repriced in July 2026 by the [GPT-5.6 model-selection guide](https://gist.github.com/lrehmann/ec36cc83f19bdf85b9f3ea19f02c9727), put Luna at 33 / 38 / 46 / 49 / 51 for `low` / `medium` / `high` / `xhigh` / `max` at roughly $0.008 / $0.010 / $0.018 / $0.028 / $0.042 per task, monotonic all the way up. At launch pricing, [Artificial Analysis](https://artificialanalysis.ai/articles/gpt-5-6-has-landed) measured Luna at `max` at about 80% below Sol's cost per task, and 75 versus Sol's 80 on the Coding Agent Index; after the July 2026 Luna repricing (an 80% price cut) the same guide puts Luna at `max` at about 4% of Sol at `max` per task, for about 86% of its Intelligence Index score, and equal to Sol at `low` on that index. The lane is not interactive: it runs detached under the 59-minute cap and the architect verifies every diff anyway, so the cost of a shallow answer (a corrected spec, a second run, an escalation to Sol) dwarfs the extra reasoning tokens, and the wall-clock spent thinking is cheap. Known costs of `max`, which the spec contract reduces but does not remove: longer time to first token (Artificial Analysis measured about 136 s), roughly twice the output tokens of `high`, and faster consumption of the context window. [PostTrainBench](https://arxiv.org/pdf/2603.08640) found `medium` beat `high` on GPT-5.1 Codex-Max, a different model, for exactly that reason on very long agentic runs, and the [OrcaRouter Luna Max playbook](https://www.orcarouter.ai/blog/gpt-5-6-luna-max-codex-playbook) notes the community default for a Luna worker had been `xhigh`; the answer is to keep every Luna spec scoped to one coherent change with one verification command, not to lower the effort. Sol and Astra keep a per-task effort because their dollars are not small.

Luna has no `ultra`; a task that seems to need `ultra` is a task for Sol. If you omit the effort on a Sol or Astra spec, the lane runs codex at the user's own configured default and flags that in `GAPS` — acceptable for trivial work, never for an escalation. `data-investigator`, the exploration agents, and `fable-advisor` are Claude agents and inherit the session effort; a `REASONING` line sent to them is ignored.
...
6. **Reasoning** — one line, `REASONING: <effort>`, chosen from the table above. `codex-implementer` ignores it and runs `max`; it binds `sol-implementer` and `astra-advisor`
