...
4. **Constraints** — project conventions, things not to touch. The lane preamble already forbids installs, dependency fetches, generated-artifact builds, and service restarts the spec does not name; when the task needs one (a release build the runtime preloads, a watcher restart), name it here so the lane runs it instead of stopping.
