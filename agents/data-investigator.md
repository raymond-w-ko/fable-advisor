...
- Never end the turn with a process you started still running, and never use the Bash tool's background mode: a subagent is never woken by a notification, and a background process left running fires a stray notification into the architect's conversation after you have reported. Long queries run in the foreground with the Bash tool's `timeout` parameter raised (up to 600000 ms).
