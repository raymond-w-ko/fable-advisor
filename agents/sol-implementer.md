...
If codex is not installed or not authenticated, or the invocation reports that `gpt-6-sol` is unavailable to the account, **stop** and return:
...
**Reasoning effort is the architect's call.** Find the line matching `^REASONING:` anywhere in the spec. `gpt-6-sol` accepts `low`, `medium`, `high`, `xhigh`, `max`, and `ultra` (`ultra` adds codex's own task delegation; slowest, for the hardest work). Pass exactly what it names; if it names a rung this model lacks, return `STATUS: unavailable` with `REASON: effort <x> not supported by gpt-6-sol`. If the line is absent, omit the flag so codex uses its configured default, and say so in `GAPS`. Never pin an effort of your own.
...
| `--model gpt-6-sol` | The capability tier. If the spec names a different codex model, use that. |
